// app/(dashboard)/vehicles/[vehicleId]/images/page.tsx
"use client";
...
export default function VehicleImagesPage() {
  ...
  async function handleUpload() {
    ...
    try {
      setUploading(true);
      setFileError(null);
      const updated = await addVehicleImages(vehicleId, pendingFiles);
      setVehicle(updated);
      setPendingFiles([]);
    } catch (err) {
      setFileError(getErrorMessage(err, "Couldn't upload images."));
    } finally {
      setUploading(false);
    }
  }
  ...
  async function handleSetPrimary(imageId: string) {
    try {
      ...
      setActionError(getErrorMessage(err, "Couldn't set primary image."));
    } finally {
      setBusyImageId(null);
    }
  }
  ...
  async function handleDelete(imageId: string) {
    try {
      ...
      setActionError(getErrorMessage(err, "Couldn't delete image."));
    } finally {
      setBusyImageId(null);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-xl font-bold text-text">Manage Images</h1>
      ...
      <div className="rounded-xl border border-border bg-surface p-5 shadow-sm">
        <h3 className="mb-4 text-sm font-semibold text-text">
          Current Images {vehicle.images.length > 0 && `(${vehicle.images.length})`}
        </h3>

        {vehicle.images.length === 0 ? (
          <p className="text-sm text-subtle">No images uploaded yet.</p>
        ) : (
                ...
                <div className="absolute inset-x-0 bottom-0 flex justify-between gap-1 bg-black/60 p-1.5 opacity-0 transition-opacity group-hover:opacity-100">
                  {!img.isPrimary && (
                    <button
                      ...
                      className="flex-1 rounded bg-white/90 px-1.5 py-1 text-[10px] font-medium text-text hover:bg-white disabled:opacity-50"
                    >
                      Set Primary
                    </button>
                  )}
      ...
      {/* Upload new */}
      <div className="rounded-xl border border-border bg-surface p-5 shadow-sm">
        <h3 className="mb-4 text-sm font-semibold text-text">Upload New Images</h3>
